...
# -- the shortest are Ate, Ida and Oda, all three letters -- so such a string is always a
# leftover: half of a split designation ("1977 UB" -> "UB"), a star-catalog prefix (BD, WR,
# HV), or an aperture or filter code. Digits are excluded from the pattern so that a bare
# minor-planet number is untouched.
#
# Querying one costs a network round trip whose "not found" reply is then cached forever,
# because mpc_query_by_name writes the response before checking that it is valid. Nothing
# is lost by refusing: across the corpus these resolved no body at all.
...
def minor_planet_identifiers(strings, *, logger=None):
    ...
    # Strings too short to name a minor planet are dropped before the MPC sees them. They
    ...
    rejected = {s for s in formatted if _TOO_SHORT_REGEX.match(s)}
    if rejected:
