...
The importable package is **`targets/`** (not `src/`). `pyproject.toml` sets `packages.find where = ["."]` / `include = ["targets", "targets.*", "programs"]` and pytest `pythonpath = ["."]`, so everything is imported under the package, e.g. `from targets import identify_targets`, `from targets.mpc_tools import mpc_packing`.
...
- `programs/` — maintenance / diagnostic scripts (`identify_visit.py`, `update_cometdb.py`, `update_target_xml_cache.py`, `build_spt_tests.py`, `retrieve_mast_moving_target_spts.py`, `reality_check_radec.py`). Shipped in the wheel, but most assume a source checkout (they read `tests/SPT_TESTS.py` or `caches/`); see `docs/programs.md`.
