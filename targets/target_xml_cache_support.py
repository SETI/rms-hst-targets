...
during testing), `use_local_xml_dir()` activates an overlay directory: within that context
"_local" files and the effective, merged `$LOOKUP.pickle` are written to the overlay,
reads resolve overlay-first, and the committed cache is left untouched.
"""
...
# None disables the overlay, so everything resolves to the committed cache as before.
...
def use_local_xml_dir(path: str | pathlib.Path | None):
    ...
    Parameters:
        path: The overlay directory (created if necessary), or None to disable the overlay.
    """
