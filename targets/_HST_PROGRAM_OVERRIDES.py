...
##########################################################################################
"""Lookup table of corrections and additions to information in the SPT headers.

The key is the TARG_ID (e.g., "13936_1") for which information is to be added.
Alternatively, you can replace the trailing digit by an asterisk (e.g., "13936_*", in
which case the information is added to every observation of the program.

The returned value is a dictionary, in which values given here update the values in the
original SPT header.

Several special new, lower-case keys are also supported:

* `"dict"`: The value is given as a dictionary, which is to be used to describe an
  additional target body. This is used to add targets that are not otherwise generated by
  this module.
* `"addition"`: The value is given as a single string that will be used in the target
  identification procedure to identify an additional target. For example, the value
  `"D/1993 F2 (Shoemaker-Levy 9)"` is used below to add SL9 as an additional target to the
  observations of its impact into Jupiter.
* `"done"`: If defined and True, the existing header will not be processed further.
* `"reject"`: If defined and True, this target will be rejected by the pipeline. Use this
  for observations that are not planetary.
"""
...
_HST_PROGRAM_OVERRIDES = {
    ...
    # Comet Siding Spring encounter" (STD=MARS); the comet C/2013 A1 (Siding Spring) is
    # the science target but appears only in TARDESCR, where the general identification
    # path cannot safely recover it. Supplied as an added 'dict' (no 'done' flag) so Mars
    # is still identified and the comet is appended, matching the context product produced
    # when C/2013 A1 is identified normally in program 13936's dedicated comet visits.
    # Scoped to 13936_1, not '13936_*', so the program's plain-Mars target (13936_3) stays
    # Mars only.
    '13936_1' : {'addition': 'C/2013 A1'},

    # Program 5642 (Storrs), "Observations of the Impact of Comet Shoemaker-Levy (1993E)
    # and Jupiter", is the HST campaign that monitored Jupiter (STD=JUPITER) before,
    # during, and after the 1994 July 16-22 fragment impacts. The comet is never named in
    # these headers, so the standard-body path returns only Jupiter. Add the parent comet
    ...
    # alike -- so Jupiter (and, for the torus visits, the Io torus) is still identified
    # and the comet appended. The 'WAVE' wavelength-calibration exposures also carry
    # STD=JUPITER and already resolve to Jupiter, so they receive the comet consistently.
    '5642_*'  : {'addition': 'D/1993 F2'},
}
