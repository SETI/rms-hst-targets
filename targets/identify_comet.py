...
def identify_comet(strings, elements=None, *, rms=0.1, confidence=0, logger=None):
    """Identify one or more comets by a name or list of alternative names plus optional
    orbital elements.
    ...
    Parameters:
        ...
        confidence (int, optional): A vlue 0-9 indicating the level of confidence that
            this item is a comet. A value > 5 indicates that this is _probably_ a comet;
            this affects how discrepancies are reported and logged.
        logger (Logger, optional): Logger to use.

    Returns:
        tuple: `(comet_dict, rms, valid)`:

        * `comet_dict` (dict[str] or None): A dictionary containing the attributes of a
          comet if one was identified; None otherwise.
        * `rms` (float): The fractional root-mean-square residual of the orbital elements
          if a comet was identified; zero otherwise.
        * `valid` (bool): True if a comet was matched and the RMS threshold was met.
    """
    ...
    # Query by strings
    comets, used, unused, _status = _identify_comet_by_strings(strings)
    ...
    if len(comets) == 1:
        ...
        if has_elements:
            ...
            if rms_test > rms:
                logger and logger.error(f'Orbit residual {rms_test:.4f} exceeds '
                                        f'threshold of {rms}')
                valid = (confidence > 5)
            else:
...
def _identify_comet_by_strings(strings, *, logger=None):
    ...
    _formatted, unused, _confidence = _comet_identifiers(strings)
    ...
    for string in strings:
        test = cometdb.query_comet_by_name(string, logger=logger)
        if test:
...
def _comet_identifiers(strings):
