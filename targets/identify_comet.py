...
def identify_comet_by_strings(strings, *, logger=None):
    ...
    Returns:
        ...
        * `comet_dicts` (list[dict]): Zero or more dictionaries containing the attributes
          of a comet. Multiple values indicates that the strings are either contradictory
          (identifying different comets) or ambiguous.
        * `used` (dict[str,  | list]): A dictionary keyed by each recognized string,
          returning either a comet dictionary or, if ambiguous, a list of comet
          dictionaries.
